...
//		String phone = jsonObject.getString("phone");
        ...
        if (oConvertUtils.isEmpty(workNo)) {
             workNo = this.getDate() + String.valueOf(random.nextInt(9000) + 1000);
        }
        while (sysUserService.getUserByWorkNo(workNo)!=null){
            workNo=this.getDate()+String.valueOf(random.nextInt(9000)+1000);
        }
...
//        SysUser sysUser4 = sysUserService.getUserByWorkNo(workNo);
//        if (sysUser4 != null) {
//            result.setMessage("工号已注册");
//            result.setSuccess(false);
//            return result;
//        }
//        if(null == code){
...
//			user.setPhone(phone);
            ...
            user.setRelTenantIds(relTenantIds);
			sysUserService.addUserWithRole(user,"1337369556699807745");//默认临时角色 test
            sysUserService.addUserWithDepart(user, departId);  //设置部门id
			result.success("注册成功");
		} catch (Exception e) {
    ...
    public Result<?> checkToken(@RequestParam("token") String token) {
        ...
        boolean key = redisUtil.hasKey(CommonConstant.PREFIX_USER_TOKEN+token);
        if (!key){
            return Result.error("token失效");
        }else {
            return Result.ok("token未失效");
        }
    }
