...
import com.ontoweb.newworkflow.core.model.task.*;
...
import com.ontoweb.newworkflow.core.process.handler.AssignmentHandler;
import com.ontoweb.newworkflow.core.process.handler.CountersignHandler;
...
import com.ontoweb.newworkflow.core.process.node.calendar.BusinessCalendar;
import com.ontoweb.newworkflow.core.process.node.exception.AssignExprNullException;
import com.ontoweb.newworkflow.core.process.node.exception.AssignerNotFoundException;
import com.ontoweb.newworkflow.core.process.node.exception.SwimlaneNotExistException;
import com.ontoweb.newworkflow.core.process.node.exception.UnsupportAssignmentException;
...
import com.ontoweb.newworkflow.handler.dao.TaskParticipatorDao;
...
/**
 * @author ：Ethan yan
 * @date ：Created in 2021/7/28 上午11:10
 * @description：任务节点
 * @modified By：
 * @version: $
 */
...
public class TaskNode extends Node{
    ...
    private int processAllowSpecifyAssignee(Context context, ProcessInstance processInstance) {
        TaskService taskService=(TaskService)context.getApplicationContext().getBean(TaskService.class);
...
//                session.save(task);
    ...
    private TaskReminder buildDueAction(DueDefinition dueDef,Date startReminderDate,Context context,Task task){
        ...
        BusinessCalendar businessCalendar = SpringBootBeanUtil.getBean(BusinessCalendar.class);
    ...
    private void retriveAssignees(List<Assignee> assigneeList,Context context,ProcessInstance processInstance,List<String> users) {
        ...
        for(Assignee assignee:assigneeList){
            log.info("assignee.getProviderId()123:{}" + assignee.getProviderId());
