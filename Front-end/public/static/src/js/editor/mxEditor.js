 ...
 */
mxEditor.prototype.propertiesResource = mxClient.language != 'none' ? '属性面板' : ''
 ...
 * Default is 20.
 */
mxEditor.prototype.tasksTop = 80
...
function addWareTable(that, url, params) {
  that.postDiagram(url, JSON.stringify(params), function(res) {
...
mxEditor.prototype.save = function(
  url = `${VUE_APP_API_BASE_URL}/online/cgform/api/form/dfa71e6082044ec59a8f5dda5a7e9695`,
  linefeed
) {
  // Gets the URL to post the data to
  url = url || this.getUrlPost()

  let superQueryParams = escape(
    JSON.stringify([
      {
        rule: 'eq',
        type: 'text',
        val: window.tenant_id,
        field: 'tenant_id'
      }
    ])
  )
  ...
  if (url != null && url.length > 0) {
    ...
    // return
    var cellList = []
    let getCellChilds = function(cell) {
      let num = cell.getChildCount()
      if (num <= 0) {
        return
      }
      for (let i = 0; i < num; i++) {
        // 获取cell的数据
        cellList.push(cell.getChildAt(i))
        getCellChilds(cell.getChildAt(i))
      }
    }
    let cell = this.graph.getModel().getRoot()
    getCellChilds(cell)

    let _this = this
    var waredomain = function() {
      _this.getDiagram(
        `${VUE_APP_API_BASE_URL}/online/cgform/api/getData/1464fd461cf84acfa700ca4b40294b1d?pageSize=100000&superQueryParams=${superQueryParams}`,
        function(res) {
          if (res.success && res.result) {
            const { records } = res.result
            form.addSelect('set_module_domain', '课件领域:', '课件领域', records, 'ware_domain_name')
          }

          // 父级遮罩层需要一个点击事件，点击不是content内容部分，需要收起面板
          marksbox.addEventListener('click', function(e) {
            if (
              e.target &&
              (!e.target.getAttribute('id') ||
                (e.target.getAttribute('id') != 'set_module_type_content' &&
                  e.target.getAttribute('id') != 'set_module_domain_content'))
            ) {
              let selectItem = document.querySelectorAll('#selectItem')
              if (selectItem) {
                selectItem.forEach(function(node) {
          ...
          form.addButtons(
            function() {
              // 这个时候是调接口
              let ware_domain_module = document.getElementById('set_module_domain_content')
              let ware_type_module = document.getElementById('set_module_type_content')

              let ware_title = document.getElementById('ware_name')

              if (!ware_title.value) {
                ware_title.setAttribute('placeholder', '请输入课件名称')
                return
              }
              if (!ware_type_module.getAttribute('value')) {
                ware_type_module.innerText = '请选择课件类型'
                return
              }
              if (!ware_domain_module.getAttribute('value')) {
                ware_domain_module.innerText = '请选择课件领域'
                return
              }

              let ware_doamin = {
                id:
                  ware_domain_module && ware_domain_module.getAttribute('value')
                    ? ware_domain_module.getAttribute('value')
                    : '',
                ware_domain_name: ware_domain_module.innerText ? ware_domain_module.innerText : '',
                tenant_id: window.tenant_id
              }
              let ware_type = {
                id:
                  ware_type_module && ware_type_module.getAttribute('value')
                    ? ware_type_module.getAttribute('value')
                    : '',
                ware_type_name: ware_type_module.innerText ? ware_type_module.innerText : '',
                tenant_id: window.tenant_id
              }
              let ware_name = ware_title.value

              console.log('人员信息', window.userinfo)

              let wareparams = {
                sys_org_code: window.sys_org_code,
                tenant_id: window.tenant_id,
                teacher_id: window.userinfo.id,
                teacher_name: window.userinfo.realname,
                teacher_work_no: window.userinfo.workNo,
                ware_content: data,
                ware_name: ware_name
              }
              // 存储课件信息表
              let _that = that
              that.postDiagram(
                `${VUE_APP_API_BASE_URL}/online/cgform/api/form/22f5f24e82444c3db7228f2c2412654d`,
                JSON.stringify(wareparams),
                function(res) {
                  if (res.success && res.result) {
                    const { result } = res // 课件id
                    // 调接口 课件属于XX类型

                    addWareTable(
                      _that,
                      `${VUE_APP_API_BASE_URL}/online/cgform/api/form/ed413176867946fbb8e7ffeab9d13817`,
                      {
                        tenant_id: ware_type.tenant_id,
                        ware_id: result,
                        ware_type_id: ware_type.id,
                        ware_type_name: ware_type.ware_type_name
                      }
                    )
                    // 调接口 课件属于XX领域
                    addWareTable(
                      _that,
                      `${VUE_APP_API_BASE_URL}/online/cgform/api/form/0c0982cecce847099a9241c8f58b7142`,
                      {
                        tenant_id: ware_doamin.tenant_id,
                        ware_domain_id: ware_doamin.id,
                        ware_domain_name: ware_doamin.ware_domain_name,
                    ...
                    for (let i = 0; i < cellList.length; i++) {
                      let cellnode = cellList[i]
                      let nodemodule = cellnode.getModule()
                      if (nodemodule) {
                        // 运行这个课件组件的所需要的角色，组件与角色是一对多的关系，所以需要一个专门的表
                        let noderole = cellnode.getRole()
                        if (noderole && noderole.length) {
                          // 调接口 课件属于XX领域
                          for (let i = 0; i < noderole.length; i++) {
                            addWareTable(
                              _that,
                              `${VUE_APP_API_BASE_URL}/online/cgform/api/form/54adfcc612f24fd78771814d9e6af84b`,
                              {
                                bean_id: nodemodule.id,
                                bean_name: nodemodule.value,
                                role_id: noderole[i].id,
                                role_name: noderole[i].title,
                                sys_org_code: nodemodule.sys_org_code,
                                tenant_id: nodemodule.tenant_id
                              }
                            )
                          }
                        }

                        // 课件与课件组件关系，一个课件会包含多个组件，所以需要新建一个表
                        addWareTable(
                          _that,
                          `${VUE_APP_API_BASE_URL}/online/cgform/api/form/0998b678e3cd403796f51403f3ceb9ef`,
                          {
                            bean_id: nodemodule.id,
                            bean_name: nodemodule.value,
                            tenant_id: nodemodule.tenant_id,
              ...
              marks.parentNode.removeChild(marks)

              const mxgesign = document.querySelector('.mxgesign')
              mxgesign.style.display = 'block'
    ...
    if (!marksbox) {
      marksbox = this.createMarks()
      var form = new mxForm('marks_content')
      form.addHeader('marsk_header', '选择类型和领域')
      form.addText('课件名称:', 'ware_name')

      this.getDiagram(
        `${VUE_APP_API_BASE_URL}/online/cgform/api/getData/474400feca3a4ac4a40549b57788b6ad?pageSize=100000&superQueryParams=${superQueryParams}`,
        function(res) {
          if (res.success && res.result) {
            const { records } = res.result
            form.addSelect('set_module_type', '课件类型:', '课件类型', records, 'ware_type_name')
          }
          waredomain()
        }
      )
      // form.addSelect('课件类型', [])
      marksbox.appendChild(form.getTable())
    } else {
      marksbox.style.display = 'block'
    }

    // this.postDiagram(url, JSON.stringify({
    //   courseId: '1',
    //   xml: linefeed,
    //   sys_org_code: '1111',
    //   bean_name: '添加test'
...
mxEditor.prototype.createMarks = function() {
  let markbox = document.createElement('div')
  markbox.setAttribute('id', 'marks')
  let body = document.getElementsByTagName('body')
  body[0].appendChild(markbox)
  return markbox
}
mxEditor.prototype.hideMarks = function(marksbox) {
  marksbox.style.display = 'none'
}
/**
 ...
 * (end)
 */
mxEditor.prototype.postDiagram = function(url, data, onload) {
  // if (this.escapePostData)
  // {
  // 	data = encodeURIComponent(data);
  // }
  // mxUtils.post(url, this.postParameterName+'='+data,
  mxUtils.post(url, data, function(res) {
    onload(JSON.parse(res))
  })
}
mxEditor.prototype.getDiagram = function(url, onload) {
  let req = mxUtils.get(url, function(res) {
    onload(JSON.parse(res))
  })
}
...
mxEditor.prototype.createProperties = function(cell) {
  ...
  console.log('判断是否为node', model.isConnectable(cell), model.isEdge(cell), model.isVertex(cell))
  if (mxUtils.isNode(value) && !model.isEdge(cell) && model.isVertex(cell)) {
    ...
    if (model.isVertex(cell)) {
      ...
      // 	widthField = form.addText('宽', geo.width)
      // 	heightField = form.addText('高', geo.height)
      // }
    }
    ...
    for (var i = 0; i < attrs.length; i++) {
      ...
      if (attrs[i].nodeName == 'label') {
        texts[i] = form.addText('节点名称:', val, '', val)
      }
    }
    var okFunction = mxUtils.bind(this, function() {
      // Hides the dialog
      // 获取选择的组件
      let select = document.getElementById('set_module_content')
      cell.setModule({
        id: select.getAttribute('value') ? select.getAttribute('value') : '',
        value: select.innerText,
        sys_org_code: select.getAttribute('sys_org_code') ? select.getAttribute('sys_org_code') : '',
        tenant_id: select.getAttribute('tenant_id') ? select.getAttribute('tenant_id') : ''
      })

      // 获取选中的角色
      let checkboxs = document.querySelectorAll('.person_checkbox')
      let add_person = []
      for (let i = 0; i < checkboxs.length; i++) {
        if (checkboxs[i].checked) {
          add_person.push({
            id: checkboxs[i].defaultValue,
            title: checkboxs[i].parentNode.previousSibling.firstChild.nodeValue
              ? checkboxs[i].parentNode.previousSibling.firstChild.nodeValue
              : ''
          })
        }
      }
      cell.setRole(add_person)
      ...
      try {
        ...
        for (let i = 0; i < attrs.length; i++) {
          if (attrs[i].nodeName == 'label') {
            let edit = new mxCellAttributeChange(cell, attrs[i].nodeName, texts[i].value)
            model.execute(edit)
          }
        }
    ...
    if (model.isConnectable(cell)) {
      // 调角色接口
      let _this = this
      var getPerson = function() {
        _this.getDiagram(`${VUE_APP_API_BASE_URL}/sys/role/list?pageSize=100000`, function(res) {
          if (res.success && res.result) {
            const { records } = res.result
            form.addPerson('add_person', records, cell.getRole())
          }
      ...
      // 此处还需要调组件接口
      this.getDiagram(
        `${VUE_APP_API_BASE_URL}/online/cgform/api/getData/dfa71e6082044ec59a8f5dda5a7e9695?pageSize=100000`,
        function(res) {
          if (res.success && res.result) {
            const { records } = res.result
            form.addSelect(
            ...
            // 父级遮罩层需要一个点击事件，点击不是content内容部分，需要收起面板
            let graph = document.querySelector('#properties')
            if (graph) {
              graph.addEventListener('click', function(e) {
                if (e.target && (!e.target.getAttribute('id') || e.target.getAttribute('id') != 'set_module_content')) {
                  let selectItem = document.querySelectorAll('#selectItem')
                  if (selectItem) {
                    selectItem.forEach(function(node) {
                      if (node.style.display == 'block') {
