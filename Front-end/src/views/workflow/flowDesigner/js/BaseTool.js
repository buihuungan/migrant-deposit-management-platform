...
export default class BaseTool extends Tool{
    getCommonProperties(target){
        ...
        const eventGroup=$(`<div class="uflo-group form-group" title="一个实现了com.bstek.uflo.process.handler.NodeEventHandler接口配置在Spring中bean的id，一旦配置在流程进入及离开该节点时会触发这个实现类" style="margin-top:20px"><label>事件Bean：</label></div>`);
