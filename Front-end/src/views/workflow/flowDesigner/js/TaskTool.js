...
import FormSelectDialog from '../dialog/FormSelectDialog.js';
...
export default class TaskTool extends BaseTool{
    ...
    buildTaskBasicConfig(target){
        ...
        const urlGroup=$(`<div class="form-group uflo-group"><label>表单内容：</label></div>`);
        //group.append(urlGroup);
        const tip="提示";
        const urlText=$(`<input type="text" id="url-text" class="form-control uflo-text-editor" title="${tip}" placeholder="请为当前节点选择表单内容" style="width: 220px">`);
    ...
    buildSpecifyAssignTable(target){
        ...
        const table=$(`<table class="table table-bordered">
            <thead>
            <tr style="background: #fbfbfb"><td style="width: 120px">用户名或角色名</td><td style="width: 40px">删除</td></tr>
            </thead>
        </table>`);
        group.append(table);
        const body=$(`<tbody class="111111"></tbody>`);
        addButton.click(()=>{
            assignProviderDialog.show(target.assignees,(data)=>{

                console.log('结束了提交',data)
                ...
                assignees.forEach(assign => {
                    console.log("运行到这里AAAAAAA");
                    const tr=$(`<tr><td>${assign.name}</td></tr>`);    
                    const delTD=$(`<td></td>`);
                    tr.append(delTD);
        ...
        console.log("sssssssssss",body);
        table.append(body);
        if(target.assignmentType==='Assignee'){
            console.log("运行到这里BBBBBBBBB");
    ...
    buildPage(body,pageIndex,pageSize,assignees,totalButton,totalGroup){
        ...
        for(let i=0;i<len;i++){
            if(i>=(startRow-1) && i<=(endRow-1)){
                const tr=$(`<tr><td>${assignees[i].name}</td></tr>`);
                body.append(tr);
                const delTD=$(`<td></td>`);
                tr.append(delTD);
                const del=$(`<i class="glyphicon glyphicon-trash" style="color: red;font-size: 12pt;cursor: pointer"></i>`);
    ...
    buildTab(target){
        ...
        if(target.url!=undefined){
            taskBasic.append(this.showFormInform(target));
            console.log(target);
        }
        ...
        taskForm.append(this.buildTaskForm1(target));
    ...
    showFormInform(target){
        const group = $(`<div class="form-group uflo-group"></div>`);
        // const addButton = $(`<button class="btn btn-primary" type="button" style="margin-bottom: 3px;float: right">添加表单</button>`);
        // group.append(addButton);
        //表单名称下拉框
        var showformNameValue="";
        const showForm = $(`<div class="modal-body">表单名称:<input type="text" id="showformName" value="${showformNameValue}" readonly="readonly"></input></div>`);  
        var data = new FormData()
        data.append('current', 1)
        data.append('size', 22)
        data.append('deleteFlag', 1)
        ...
        t_postAction("/admin/desform/search", data).then(res => {
            let showformNameDom = $('#showformName')
            for (let i = 0; i < res.result.list.length; i++) {
                if(res.result.list[i].id==target.url){
                    showformNameDom[0].value=res.result.list[i].name
                    target.formName=res.result.list[i].name 
                }
            }
        })
        group.append(showForm);
        return group;
    }

    buildTaskForm1(target){
        console.log("buildTaskForm1",target);
        console.log("buildTaskForm1",target.formName);
        const group = $(`<div class="form-group uflo-group"></div>`);
        const urlGroup=$(`<div class="form-group uflo-group" style="margin-top: 20px;"><label>表单名称:</label></div>`);
        group.append(urlGroup);
        const tip="选择表单";
        const urlText=$(`<input type="text" id="url-text"  value="${target.formName ||target.url|| ''}" class="form-control uflo-text-editor" title="${tip}" placeholder="请为当前节点选择表单名称" style="width: 220px">`);
        urlGroup.append(urlText);
        urlText.change(function(){
            target.formName=$(this).val();
        });
        const FormSelectButton=$(`<button type="button" class="btn btn-default" style="height: 26px;padding-top: 2px;margin-left: 2px;">选择</button>`);
        urlGroup.append(FormSelectButton);
        const dialog=new FormSelectDialog(`${tip}`);
        FormSelectButton.click(function(){
            dialog.show(function(form){
                target.formName=form.name  
                target.url=form.name
                urlText.val(target.formName);
            });
        });
        return group;
    }
