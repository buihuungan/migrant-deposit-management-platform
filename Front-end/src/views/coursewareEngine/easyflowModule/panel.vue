  ...
  <div>
    ...
    >
      <div v-if="easyFlowVisible" id="easyflow">
              ...
              <div class="header-content flex-col">
                <div class="header-content1 flex-row">
                  ...
                  <div class="header-content-save flex-col" @click="saveFlow">
                    <span class="header-content-save-info">保存流程</span>
                  </div>
                </div>
              </div>
              <!--顶部工具菜单-->
        ...
        <saveMessage ref="saveMessage" :data="data" @closeEasyflow="closeEasyflow"></saveMessage>
      </div>
    </a-modal>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      // 数据
      data: {},
      edu_ware_ware_domain:[],
      edu_ware_ware_type:[],
      edu_ware_ware_industry:[],
      // ware_industry_name: [],
      // 激活的元素、可能是节点、可能是连线
      activeElement: {
        // 可选值 node 、line
  ...
  methods: {
    ...
    jsPlumbInit() {
      this.jsPlumb.ready(() => {
        ...
        this.jsPlumb.bind('beforeDrop', (evt) => {
          ...
          if(this.berectangle(from,to)){
            this.$message.error('选择节点的前一个节点必须是课程名称，且课程名称的功能类型必须是表单')
            return false
          }
    ...
    addNode(evt, nodeMenu, mousePosition) {
      ...
      var left = screenX,
        top = screenY
        console.log("2222",screenX,screenY,efContainer,containerRect,left,top)
      // 计算是否拖入到容器中
      if (
      ...
      switch (type) {
        ...
        case 'task':
          var node = {
            ...
            // role: [],
            task_type: 'Normal',
            assignment_type: 'Assignee', // 如果需要制定参与者则需要做如下信息
            assignee: [], //参与者
            state: nodeMenu.state,
            feature: {},
            formDesignerId: '',
      ...
      console.log("78533333",node)
      console.log("asfaweqw",this.data.nodeList)
      /**
       * 这里可以进行业务判断、是否能够添加该节点
       */
      
      // this.data.nodeList.forEach((item)=>{
      //   console.log("6655",item.type,item.name,node.name)
      //   if(node.type == 'swinlane'&& item.name == node.name){
      //       this.$message.error('该公司已存在，请重新选择公司')
      //       flag = 1
      //   }
      // })
      if(node.type == 'swinlane' ){
        this.data.nodeList.unshift(node)
      }else{
    ...
    //判断菱形节点之前是不是长方形节点，且长方形节点任务类型必须是表单
    berectangle(from,to){
      var fromNode2 = {}
      var toNode2 = {}
      for (let i = 0; i < this.data.nodeList.length; i++) {
      ...
      console.log("fslghiwh",fromNode2.feature.type,fromNode2.type,toNode2.feature.type,toNode2.type)
      if(toNode2.type == 'rhombus'){
          if(fromNode2.type == 'task' && fromNode2.feature.type == "表单" ){
            console.log("sdf",true)
            return false
    ...
    saveFlow() {
      ...
      if (this.saveFlag == '0') {
        ...
        this.$refs.saveMessage.loadData()
      } else {
        this.$refs.saveMessage.openModel()
        this.$refs.saveMessage.loadData()
        console.log('保存的流程', this.data)
        // this.edu_ware_ware_industry = [{ware_industry_name : [this.data.types[2]] , sys_tenant_id: vue.ls.get(TENANT_ID)}]
        // this.edu_ware_ware_domain = [{ ware_domain_name:this.data.domains , sys_tenant_id: vue.ls.get(TENANT_ID)}]
        // this.edu_ware_ware_type = [{ware_type_name:this.data.types , sys_tenant_id: vue.ls.get(TENANT_ID)}]
        // ce_postAction1(`/design/saveFile`, {
        //   name: this.data.name,
        //   content: JSON.stringify(this.data),
        //   edu_ware_ware_type: this.edu_ware_ware_type,
        //   edu_ware_ware_domain :this.edu_ware_ware_domain,
        //   edu_ware_ware_industry :this.edu_ware_ware_industry,
        //   description :  '11',
        //   update: 'true',
