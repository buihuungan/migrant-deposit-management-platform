    ...
    <a-form>
      ...
      >
        ...
        >
          ...
          <a-select-option v-for="d in tenantList" :key="d.id" :value="d.id">{{ d.name }}</a-select-option>
        </a-select>
      </a-form-item>
...
export default {
  ...
  methods: {
    ...
    //   }
    // },
    show(loginResult,loginParams) {
      console.log(loginResult)
      this.clear()
      this.visible = true
      this.isMultiTenant = true
      this.tenantList = loginResult
      // for(let index = 0;index<loginResult.length;index++)
      // {
      //   this.tenantList.push(loginResult[index].name) 
      // }
      console.log("this.tenantList",this.tenantList)
      // this.bizDepart(loginResult)
      // console.log('ssss:', loginParams)
