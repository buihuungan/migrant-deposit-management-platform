...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goPageOnline(rd) {
        if(rd.isTree=='Y'){
          this.$router.push({ path: '/cgformTreeList/' + rd.id })
        }else{
          this.$router.push({ path: '/cgformList/' + rd.id })
        }
      },
      ...
      handleOnlineUrlShow(record) {
        if(record.isTree=='Y'){
          this.onlineUrl = `/cgformTreeList/${record.id}`
        }else{
          this.onlineUrl = `/cgformList/${record.id}`
        }
      ...
      doCgformButton() {
        ...
        //this.$router.push({ path: '/cgformButton/' + this.selectedRowKeys[0] })
      },
      importOnlineForm() {
