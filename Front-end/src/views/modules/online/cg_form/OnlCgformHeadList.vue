...
import { o_deleteAction, o_postAction, o_getAction } from '@/api/onApi'
...
export default {
  ...
  methods: {
    loadData(arg) {
      o_getAction('/cgform/head/list', { pageNo: this.ipagination.current, tableName: arg })
        .then((res) => {
          console.log(res)
          if (res.success) {
    ...
    doDbSynch(id) {
      o_postAction(this.url.doDbSynch + id, { synMethod: '1' }).then((res) => {
        if (res.success) {
    ...
    handleDbSync() {
      ...
      o_postAction(this.url.doDbSynch + this.syncFormId + '/' + this.synMethod).then((res) => {
        this.syncModalVisible = false
    ...
    handleRemoveRecord(id) {
      ...
      this.$confirm({
        title: '确认要移除此记录?',
        onOk() {
          o_deleteAction(that.url.removeRecord, { id: id }).then((res) => {
            if (res.success) {
    ...
    handleDelete(id) {
      let that = this
      o_deleteAction('/cgform/head/delete', { id: id }).then((res) => {
        if (res.success) {
    ...
    copyConfig(id) {
      o_postAction(`${this.url.copyOnline}?code=${id}`).then((res) => {
        if (res.success) {
          this.$message.success('复制成功')
          this.loadData()
        } else {
