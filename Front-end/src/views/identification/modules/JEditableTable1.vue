      ...
      <a-col>
        ...
        <div v-if="actionButton" class="action-button">
          ...
          <template v-if="selectedRowIds.length > 0">
            <a-popconfirm :title="`确定要删除这 ${selectedRowIds.length} 项吗?`" @confirm="handleConfirmDelete">
    ...
    <div :id="`${caseId}inputTable`" class="input-table">
      ...
      <div class="scroll-view" ref="scrollView" :style="{ 'max-height': maxHeight + 'px' }">
        ...
        <div :id="`${caseId}tbody`" class="tbody" :style="tbodyStyle">
          ...
          <!-- v-model="rows"-->
          <draggable :value="rows" handle=".td-ds-icons" @start="handleDragMoveStart" @end="handleDragMoveEnd">
            <!-- 动态生成tr -->
            <template v-for="(row, rowIndex) in rows">
              ...
              >
                ...
                <div
                  v-if="dragSort"
                  class="td td-ds"
                  :style="style.tdLeftDs"
                  @dblclick="_handleRowInsertDown(rowIndex)"
                >
                  <a-dropdown :trigger="['click']" :getPopupContainer="getParentContainer">
                    ...
                    <a-menu slot="overlay">
                      <a-menu-item key="0" :disabled="rowIndex === 0" @click="_handleRowMoveUp(rowIndex)"
                        >向上移</a-menu-item
                      >
                      <a-menu-item
                        key="1"
                        :disabled="rowIndex === rows.length - 1"
                        @click="_handleRowMoveDown(rowIndex)"
                        >向下移</a-menu-item
                      >
                      <a-menu-divider />
                ...
                <div v-if="rowSelection" class="td td-cb" :style="style.tdLeft">
                  ...
                  <template v-for="(id, i) in [`${row.id}`]">
                    <a-checkbox
                      :id="id"
                      :key="i"
                      :disabled="disableCheckbox.indexOf(rowIndex) !== -1"
                ...
                >
                  ...
                  <template v-for="(id, i) in [`${col.key}${row.id}`]">
                    ...
                    <label :key="i" v-if="col.type === formTypes.input || col.type === formTypes.inputNumber">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)" trigger="click" >
                        <input
                          v-if="isEditRow(row, col)"
                          :id="id"
                          v-bind="buildProps(row, col, rowIndex)"
                        ...
                        <span
                          ...
                          :class="{ disabled: buildProps(row, col, rowIndex).disabled }"
                      ...
                      <template v-if="uploadValues[id] != null" v-for="(file, fileKey) of [uploadValues[id] || {}]">
                        <div :key="fileKey" style="position: relative">
                          <template
                            v-if="
                              !uploadValues[id] ||
                              !(uploadValues[id]['url'] || uploadValues[id]['path'] || uploadValues[id]['message'])
                            "
                          >
                            <a-icon type="loading" />
                          </template>
                    ...
                    <!-- else (normal) -->
                    <span v-else :key="i" v-bind="buildProps(row, col, rowIndex)">{{
          ...
          >
            <div v-if="dragSort" class="td td-ds" :style="style.tdLeftDs"></div>
            <div v-if="rowSelection" class="td td-cb" :style="style.tdLeft">统计</div>
            <div v-if="rowNumber" class="td td-num" :style="style.tdLeft">
            ...
            <!-- 右侧动态生成td -->
            <template v-for="col in columns">
              <div :key="col.key" class="td" v-show="col.type !== formTypes.hidden" :style="buildTdStyle(col)">
                <span v-show="col.type === formTypes.inputNumber" style="padding: 0 5px">{{
...
export default {
  ...
  data() {
    return {
      ...
      deleteFieldIds: [],
      //当前禁用的行数
      currentTempNumber:this.tempNumber,
    }
  },
  created() {
    this.inputValues = []
    ...
    // console.log('接收到的disablerows', this.disabledRows)
  ...
  watch: {
    ...
    selectedRowIds(newValue) {
      this.$emit(
        'selectRowChange',
        cloneObject(newValue).map((i) => this.getCleanId(i))
      )
    },
  },
  mounted() {
  ...
  methods: {
      ...
      dataSource.forEach((data, newValueIndex) => {
        ...
        this.columns.forEach((column) => {
          ...
          for (let columnKey in this.disabledRows) {
            // console.log('columnKeycolumnKeycolumnKey',columnKey);
            // console.log('dartatatatata',data);
            // 判断是否有该属性
            if (this.disabledRows.hasOwnProperty(columnKey) && data.hasOwnProperty(columnKey)) {
              if (disabled !== true) {
                ...
                if (temp instanceof Array) {
                  ...
                  if (temp !== data[columnKey]) check = false
                }
                if (check) {
                  disabledRowIds.push(row.id)
                }
              }
              // console.log('disabledRowIdsdisabledRowIdsdisabledRowIds', disabledRowIds)
            }
          }
        })
        // 插入行而不是添加到最后
        ...
        if (insertIndexes instanceof Array) {
          ...
          if (typeof insertIndex === 'number') {
            ...
            // this.inputValues[insertIndex].dbType = data['dbType']
            // this.inputValues[insertIndex].dbIsKey = data['dbIsKey']
            // this.inputValues[insertIndex].dbIsNull = data['dbIsNull']
          }
        }
        if (!added) {
          rows.push(row)
          this.inputValues.push(value)
          let lastIndex = this.inputValues.length - 1
          // this.inputValues[lastIndex].dbType = data['dbType']
          // this.inputValues[lastIndex].dbIsKey = data['dbIsKey']
          // this.inputValues[lastIndex].dbIsNull = data['dbIsNull']
        }
      })
      // 启用了拖动排序，就重新计算排序编号
      if (this.dragSort) {
    ...
    add(num = 1, forceScrollToBottom = false) {
      if (num < 1) return
      // let timestamp = new Date().getTime()
      // console.log('rows111', this.rows);
      let rows = this.rows
      let row
      for (let i = 0; i < num; i++) {
        rows = this.push({}, false, rows)
        // console.log('thissss',rows);
      ...
      for (let value of inputValues) {
        ...
        this.columns.forEach((column) => {
          ...
          if (validate === true) {
            ...
            asyncCount++
            let results = this.validateOneInputAsync(
              value[column.key],
              value,
              column,
              notPassedIds,
    ...
    getAll(validate, deleteTempId) {
      return new Promise((resolve, reject) => {
        ...
        this.getValuesPromise(validate, null, deleteTempId)
          .then((values) => {
    ...
    handleChangeCheckedAll() {
      ...
      if (!this.getSelectAll) {
        this.rows.forEach((row, rowIndex) => {
          // console.log('rowrowrowrow', rowIndex)
          if (this.disableCheckbox.indexOf(rowIndex) === -1) {
    ...
    handleConfirmDelete() {
      // console.log("删除selectedRowIds",this.selectedRowIds)
      // console.log("this.selectedRowIds",this.selectedRowIds)
      // console.log("this.deleteFieldIds",this.deleteFieldIds)
      this.selectedRowIds.forEach((item, index) => {
        this.deleteFieldIds.push(item.split('-')[2])
      })
      // console.log("this.deleteFieldIds",this.deleteFieldIds)
      this.removeSelectedRows()
      this.currentTempNumber -= this.deleteFieldIds.length
      // console.log("this.currentTempNumber",this.currentTempNumber)
    },
    ...
    handleInputCommono(target, index, row, column) {
      ...
      // console.log('target, index, row, column',target, index, row, column);
    ...
    _loadDictConcatToOptions(column) {
      // console.log('加载数据字典并合并到 options')
    ...
    buildTdStyle(col) {
      const isEmptyWidth = (column) =>
        column.type === FormTypes.hidden || column.width === '0px' || column.width === '0' || column.width === 0
    ...
    /** view辅助方法：构造props */
    buildProps(row, col, rowIndex) {
      // console.log('buildprops的计数', this.tempNumber)
      ...
      if (props['disabled'] !== true) {
        ...
        props['disabled'] = rowIndex < this.currentTempNumber
      }
