        ...
        <a-form layout="inline" class="j-inline-form">
          ...
          <a-form-item :label="queryParamText || name">
            <a-input
              ...
              @pressEnter="searchQuery(isonlineUrl, searchobj, ischildtable)"
            />
          </a-form-item>
          <!-- 动态生成的查询条件 -->
          ...
          />
          <!-- 按钮 -->
          <a-button type="primary" @click="searchQuery(isonlineUrl, searchobj, ischildtable)" icon="search"
            >查询</a-button
          >
          <a-button
            type="primary"
            @click="searchReset(isonlineUrl, searchobj, ischildtable)"
            icon="reload"
...
export default {
  ...
  watch: {
    searchobj: {
      ...
      immediate: true,
      handler(val) {
        this.searchQuery(this.isonlineUrl, val, this.ischildtable)
      },
    },
    value: {
      deep: true,
  ...
  methods: {
    ...
    async queryOptionsByValue(value) {
      ...
      if (this.isonlineUrl) {
        ...
        var res = await getAction(this.valueUrl || this.listUrl, params).then()
      } else {
        var res = await getAction(this.valueUrl || this.listUrl, {
      ...
      if (res.success) {
        ...
        if (!(dataSource instanceof Array)) {
          // console.log('是这里吗',res.result.records)
