...
export default {
  ...
  methods: {
    closeMenu (e) {
      if(e.target.href && e.target.href.indexOf('stjlangchain')>0) {
        // window.open(e.target.href,'_blank')

        let url = e.target.href.replace('/stjlangchain','/llm-kg/subject-one/stj-front#/stjlangchain')
        window.open(url,'_blank')
      }
      if(e.target.href && e.target.href.indexOf('www.baidu.com')>0) {
        window.open(e.target.href,'_blank')
      }
