<template>
  <div class="form-table" :class="{ 'is-disabled': disabled }">
    ...
    <!-- <el-button icon="el-icon-plus" type="text" @click="handleAddRow" >{{ $t('fm.actions.add') }}</el-button> -->
...
export default {
  ...
  props: ['columns', 'value', 'models', 'remote', 'blanks', 'disabled', 'rules', 'name', 'remoteOption', 'jData'],
  data() {
    return {
      tableData: this.value,
      dataModels: this.models,
      jColumns: this.columns,
    }
  },
  mounted() {
    /*
  ...
  methods: {
    handleAddRow() {
      // console.log('fmmmmm', this.$t('fm.actions.add'))
      console.log('fmmmmm', '添加')
      let item = {}
    ...
    formOk(data, index) {
      this.columns.forEach((item) => {
        Object.keys(data).forEach((element) => {
          if (item.tableCol === element) {
            this.tableData[index][item.model] = data[element]
          }
        })
      })
    },
