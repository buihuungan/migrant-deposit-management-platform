    ...
    <div :id="`${caseId}inputTable`" class="input-table">
      ...
      <div class="scroll-view" ref="scrollView" :style="{'max-height':maxHeight+'px'}">
        ...
        <div :id="`${caseId}tbody`" class="tbody" :style="tbodyStyle">
          ...
          <div
            class="tr-expand"
            :style="`height:${getExpandHeight}px; z-index:${loading?'11':'9'};`"
          ></div>
          <!-- 无数据时显示 -->
              ...
              >
                  ...
                  <a-dropdown :trigger="['click']" :getPopupContainer="getParentContainer">
                    ...
                    <a-menu slot="overlay">
                      <a-menu-item
                        key="0"
                        :disabled="rowIndex===0"
                        @click="_handleRowMoveUp(rowIndex)"
                      >向上移</a-menu-item>
                      <a-menu-item
                        key="1"
                ...
                >
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <label
                      :key="i"
                      v-if="col.type === formTypes.input || col.type === formTypes.inputNumber"
                    >
                    ...
                    <!-- date -->
                    <template
                      v-else-if="col.type === formTypes.date || col.type === formTypes.datetime"
                    >
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <j-date
                          v-if="isEditRow(row, col)"
                      ...
                      >
                        <div :key="fileKey" style="position: relative;">
                          ...
                          <a-tooltip v-else :title="file.name">
                            ...
                            >{{ getEllipsisWord(file.name,5) }}</span>
                          </a-tooltip>
